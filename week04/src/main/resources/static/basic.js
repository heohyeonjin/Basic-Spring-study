let targetId; //어느 함수에서든 사용 가능

$(document).ready(function () { //새로 고침할 때 마다 해당 내용 실행
...
function execSearch() {
     ...
     */
     $('#search-result-box').empty(); //값 비워주기
    // 1. 검색창의 입력값을 가져온다.
    let query = $('#query').val();
    // 2. 검색창 입력값을 검사하고, 입력하지 않았을 경우 focus.
    if (query == ''){
        alert("검색어를 입력해주세요.");
        $('#query').focus(); //여기 입력하라고 focusing
    }
...
function addProduct(itemDto) {
    ...
    $.ajax({
        ...
        success:function(response){
            $('#container').addClass('active'); //모달 띄우기
